...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(
            self.n_features * self.embed_size, self.hidden_size)
        ))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(
            self.hidden_size,
        )))
        self.dropout = nn.Dropout(p=self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(
            self.hidden_size, self.n_classes)
        ))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(torch.empty(
            self.n_classes,
        )))
    ...
    def forward(self, w):
        ...
        embedding_res = self.embedding_lookup(w) # embedding_res (Tensor): (batch_size, n_features * embed_size)

        # self.embed_to_hidden_weight (Tensor): (n_features * embed_size, hidden_size)
        affine_res = torch.matmul(embedding_res, self.embed_to_hidden_weight) + self.embed_to_hidden_bias 
        
        h = self.dropout(F.relu(affine_res)) # affine_res (Tensor): (batch_size, hidden_size)
        
        # hidden_to_logits_weight (Tensor): (hidden_size, n_classes)
...
if __name__ == "__main__":
    ...
    embeddings = np.zeros((100, 30), dtype=np.float32) # word embeddings (num_words, embedding_size)
    model = ParserModel(embeddings)

    def check_embedding():
        inds = torch.randint(0, 100, (4, 36), dtype=torch.long) # inds (batch_size=4, n_features=36)
        selected = model.embedding_lookup(inds)
        print("selected.shpae:", selected.shape) # (batch_size=4, (n_features * embed_size)=(36 * 30)=1080)

...
class ACEBartForConditionalGeneration(bart.PretrainedBartModel):
    ...
    def get_special_ids(self, tokenizer, y, entitys):
        ...
        return event_ids, arg_ids, arg_indexes_ids, role_ids, label_dict, role_entity_type, y_args_ids, y_entity_ids, entity_arg_ids, entity_arg_dict, total_args_number, arg_candidates, entity_type_candidates, entity_type_arg_dict
    ...
    def predict_beam_search(self, input_ids,
        ...
        event_ids, arg_ids, arg_indexes, role_ids, label_dict, role_entity_type, y_args_ids, y_entity_ids, entity_arg_ids, entity_arg_dict, total_args_number, arg_candidates, entity_type_candidates, entity_type_arg_dict = self.get_special_ids(
            tokenizer, y, entitys)
        argument_count = 0
        ...
        while cur_len < 10:
            if cur_len == 1:
                ...
                next_token_logits = torch.index_select(next_token_logits, 1,
                                                       torch.tensor(classification_token_ids))
                ...
                if int(next_token) == classification_token_ids[1]:
                    ...
                    final_graph = torch.cat([new_input_ids, torch.tensor([eos_token_id]).unsqueeze(-1)], dim=-1)
                    break
                else:
                ...
                next_token_logits = torch.index_select(next_token_logits, 1, torch.tensor(event_ids))
                # next_token1 = torch.argmax(next_token_logits, dim=-1)
    ...
    def predict_beam_search_v1(self, input_ids,
                               y,
                               cur_len,
                               max_length,
                               min_length,
        ...
        tokenizer = model_specific_kwargs['tokenizer']
        entitys = model_specific_kwargs['entitys']
        event_ids, arg_ids, arg_indexes, role_ids, label_dict, y_args_ids, y_entity_ids, entity_arg_ids, entity_arg_dict, num, arg_candidates = self.get_special_ids(
            tokenizer, y, entitys)
        argument_count = 0
        y_args_ids = arg_candidates
        total_arg_num = len(y_args_ids)
        ...
        while cur_len < max_length:
            if cur_len == 1:
                ...
                if event_flag:
                    for s in stop_graphs:
                        s_l = len(s[0][0]) - 1
                        s3 = s[2] / s_l
                        s.append(s3)
                    partial_graph = self.get_topk_graphs_for_stop(stop_graphs, beam_size)
                    event_flag = 0
                else:
                    partial_graph = total_graphs
                total_graph = []
                ...
                for ids, graph in enumerate(partial_graph):
                    ...
                    if last_token_whether_event in event_ids:
                        ...
                        e_cur_len = 0
                        e_new_graphs = [graph]
                        while e_cur_len < max_length:
                            e_total_graph = []
                            graphs = e_new_graphs
                            for e_ids, e_graph in enumerate(graphs):
                                e_ids_input_ids = e_graph[0]
                                e_last_token = e_ids_input_ids[0][-1]
                                if int(e_last_token) == eos_token_id:
                                    e_total_graph.append(e_graph)
                                    continue
                                e_ids_past = e_graph[1]
                                e_ids_score = e_graph[2]
                                ...
                                if e_last_token in event_ids:
                                    if total_arg_num != 0:
                                        ...
                                        next_token_candidatas = [stop_token_id]
                                if e_last_token in role_ids and e_last_last_token in arg_indexes + arg_ids:
                                    if e_ids_argument_count >= total_arg_num:
                                        next_token_candidatas = [stop_token_id]
                                    else:
                                        next_token_candidatas = y_entity_ids + [stop_token_id]
                                ...
                                for i, next_token in enumerate(next_tokens):
                                    ...
                                    new_input_ids = torch.cat([e_ids_input_ids, tokens_to_add.unsqueeze(-1)], dim=-1)
                                    score = e_ids_score + next_score
                                    if int(next_token) in y_args_ids and e_last_token in arg_indexes:
                                        new_ids_argument_count = e_ids_argument_count + 1
                                        if int(next_token) not in e_ids_arg_list:
                                            new_list = e_ids_arg_list + [int(next_token)]
                                    else:
                                    ...
                                    e_total_graph.append(
                                        [new_input_ids, e_ids_past, score, new_ids_argument_count, new_list,
                                         ...
                                         e_ids_entity_token])
                            #
                            for e_it in e_new_graphs:
                                e_l = len(e_it[0][0]) - 1
                                e_it_score = e_it[2] / e_l
                                e_it.append(e_it_score)
                            e_new_graphs = self.get_topk_graphs(e_total_graph, beam_size)
                            e_cur_len = e_cur_len + 1
                    else:
                        stop_flag = 1
                        for ids, graph in enumerate([graph]):
                            ...
                            if ids_argument_count == 4:
                                a = 0
                            if ids_argument_count > total_arg_num:
                                final_graphs.append([new_input_ids, ids_past, score, ids_argument_count])
                                break
                            last_token = ids_input_ids[0][-1]
                            last_last_token = ids_input_ids[0][-2]
                            if last_token in event_ids:
                                if total_arg_num != 0:
                                    if ids_argument_count >= total_arg_num:  # 论元个数够了
                                        next_token_candidatas = [stop_token_id]
                                    else:
                                        ids_e_token = last_token
                                        next_token_candidatas = y_entity_ids + [stop_token_id]
                                else:  # 无论元
                                    next_token_candidatas = [stop_token_id]
                            if last_token in role_ids and last_last_token in arg_indexes + arg_ids:
                                if ids_argument_count >= total_arg_num:
                                    next_token_candidatas = [stop_token_id]
                                else:
                                    next_token_candidatas = y_entity_ids + [stop_token_id]
                            if last_token in arg_indexes:
                                if int(last_token) in ids_input_ids[0][:-1]:
                                    next_token_candidatas = label_dict[int(ids_e_token)]
                                else:  # 新的论元出现
                                    if len(y_args_ids) != 0:
                                        if entity_arg_dict[int(ids_entity_token)]:
                                            new_arg_ids = []
                                            for a in entity_arg_dict[int(ids_entity_token)]:
                                                if a not in ids_arg_list:
                                                    new_arg_ids.append(a)
                                            if len(new_arg_ids) == 0:
                                                new_arg_ids = entity_arg_dict[int(ids_entity_token)]
                                            # next_token_candidatas = entity_arg_dict[int(entity_token)]
                                            next_token_candidatas = new_arg_ids
                                        else:
                                            next_token_candidatas = y_args_ids

                            if last_token in arg_ids and last_last_token in arg_indexes:
                                next_token_candidatas = label_dict[int(ids_e_token)]
                            if last_token in y_entity_ids and last_last_token in event_ids + role_ids:
                                ids_entity_token = last_token
                                flag = 1
                                if ids_argument_count >= total_arg_num:
                                    next_token_candidatas = arg_indexes
                                else:
                            ...
                            for i, next_token in enumerate(next_tokens):
                                ...
                                if tokens_to_add == stop_token_id:
                                    pass
                                    # new_partial_graphs.append(ids_input_ids)
                                    # new_pasts.append(past)
                                # new_list=[]
                                if int(next_token) in y_args_ids and last_token in arg_indexes:
                                    new_ids_argument_count = ids_argument_count + 1
                                    if int(next_token) not in ids_arg_list:
                                        new_list = ids_arg_list + [int(next_token)]
                                else:
                                    new_ids_argument_count = ids_argument_count
                                    new_list = ids_arg_list
                                total_graph.append(
                                    [new_input_ids, ids_past, score, new_ids_argument_count, new_list, ids_e_token,
                                     ids_entity_token])

                        # total_graphs=self.get_topk_graphs(total_graph,beam_size)
                if stop_flag:
                    if len(total_graph) == 0:
                        # final_graphs=final_graphs+partial_graph
                        ...
                        if len(final_graphs) == 0:
                            final_graphs = partial_graph
                        final_graph = self.get_topk_graphs(final_graphs, 1)
                        final_graph = final_graph[0][0]
                        break
                    total_graphs = self.get_topk_graphs(total_graph, beam_size)
                cur_len = cur_len + 1
    ...
    def predict_beam_search_v2(self, input_ids,
                               y,
                               cur_len,
                               max_length,
                               min_length,
        ...
        tokenizer = model_specific_kwargs['tokenizer']
        entitys = model_specific_kwargs['entitys']
        event_ids, arg_ids, arg_indexes, role_ids, label_dict, y_args_ids, y_entity_ids, entity_arg_dict, num, arg_candidates = self.get_special_ids(
            tokenizer, y, entitys)
        argument_count = 0
        y_args_ids = arg_candidates
        total_arg_num = len(y_args_ids)
        ...
        while cur_len < 70:
            if cur_len == 1:
                ...
                for e_ids, e_graph in enumerate(graphs):
                    e_ids_input_ids = e_graph[0]
                    last_token_whether_eos = e_ids_input_ids[0][-1]
                    if int(last_token_whether_eos) == eos_token_id:
                        # if ids==0:
                        #     final_graph=graph[0]
                        #     flag=1
                        #     break
                    ...
                    e_ids_entity_token = e_graph[6]
                    model_inputs = self.prepare_inputs_for_generation(
                        e_ids_input_ids, past=e_ids_past, attention_mask=attention_mask, use_cache=use_cache,
                        **model_specific_kwargs
                    )
                    ...
                    if eos_token_id is not None and cur_len < min_length:
                        next_token_logits[:, eos_token_id] = -float("inf")
                    last_token = e_ids_input_ids[0][-1]
                    last_last_token = e_ids_input_ids[0][-2]
                    if last_token in event_ids:
                        if total_arg_num != 0:
                            if e_ids_argument_count >= total_arg_num:  # 论元个数够了
                                next_token_candidatas = [stop_token_id]
                    ...
                    if last_token in role_ids and last_last_token in arg_indexes + arg_ids:
                        if e_ids_argument_count >= total_arg_num:
                            ...
                            next_token_candidatas = y_entity_ids + [stop_token_id]
                    if last_token in arg_indexes:
                        if int(last_token) in e_ids_input_ids[0][:-1]:
                            next_token_candidatas = label_dict[int(e_ids_e_token)]
                        else:  # 新的论元出现
                            new_arg_ids = []
                            for a in entity_arg_dict[int(e_ids_entity_token)]:
                            ...
                            if len(y_args_ids) != 0:
                                if entity_arg_dict[int(e_ids_entity_token)]:
                                    if new_arg_ids:
                                        ...
                                        next_token_candidatas = entity_arg_dict[int(e_ids_entity_token)]
                                else:
                                    next_token_candidatas = y_args_ids
                            # new_argument_count =e_ids_argument_count+ 1
                    if last_token in arg_ids and last_last_token in arg_indexes:
                        next_token_candidatas = label_dict[int(e_ids_e_token)]
                    if last_token in y_entity_ids and last_last_token in event_ids + role_ids:
                        ids_entity_token = last_token
                    ...
                    next_token_logits = torch.index_select(next_token_logits, 1,
                                                           torch.tensor(next_token_candidatas))
                    next_token_probs, next_token_index = torch.topk(next_token_logits, new_beam_size, dim=1)
                    next_tokens = [next_token_candidatas[int(id)] for id in next_token_index.squeeze(0)]
                    ...
                    for i, next_token in enumerate(next_tokens):
                        ...
                        e_total_graph.append(
                            [new_input_ids, ids_past, score, new_ids_argument_count, new_list, ids_e_token,
                             ids_entity_token])
                for t in e_total_graph:
                    t_l = len(t[0][0]) - 1
                    t_score = t[2] / t_l
                    t[3] = t_score
                total_graphs = self.get_topk_graphs_for_stop(e_total_graph, beam_size)
                cur_len += 1
                    ...
                    if cur_len == 1:
                        ...
                        if cur_len == 2:
                            ...
                            if last_token in arg_ids + entity_arg_ids and last_last_token in arg_indexes:
                                new_role_list = []
                                for role in label_dict[int(e_token)]:
                                    next_token_candidatas = label_dict[int(e_token)]
                            if last_token in y_entity_ids and last_last_token in event_ids + role_ids:
